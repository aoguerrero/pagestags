...
public class PageSaveFormCtrl extends FormController {
    ...
    public PageSaveFormCtrl(String redirectPath, PagesTagsCtx ctx) {
        super(redirectPath);
        this.ctx = ctx;
    }
    ...
    public Optional<String> execute(HttpRequest request, Map<String, String> formData) {
        ...
        this.ctx.getPages().remove(new Page(id));
        List<String> tagList = Arrays.asList(tags.split(" "));
        if (newId != null && !newId.equals(id)) {
            Path newPath = Paths.get(PAGES_PATH.get(), newId).toAbsolutePath();
            FileSystemUtils.renameFile(savePath, newPath);
            this.ctx.getPages().add(new Page(newId, title, tagList, pblic != null));
            return Optional.of(newId);
        }
        this.ctx.getPages().add(new Page(id, title, tagList, pblic != null));
